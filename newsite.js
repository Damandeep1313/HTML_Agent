...
// ----------------------------------------------------
// 2) NETLIFY CREATE & DEPLOY (in-memory, no disk file)
// ----------------------------------------------------
const NETLIFY_AUTH_TOKEN = process.env.NETLIFY_AUTH_TOKEN;
if (!NETLIFY_AUTH_TOKEN) {
  throw new Error("Missing NETLIFY_AUTH_TOKEN in .env");
}

/**
 *  createNewNetlifySite:
 *  Creates a brand-new ephemeral site on Netlify
 *  using your account’s auth token. Returns site_id.
 */
async function createNewNetlifySite() {
  console.log("createNewNetlifySite: Creating ephemeral site on Netlify...");

  const url = `https://api.netlify.com/api/v1/sites`;
  // If you omit the body, Netlify will generate a random subdomain for you.
  // Or you can pass { name: "some-unique-subdomain" } if you prefer.
  ...
  // Example: resp.data.site_id, resp.data.ssl_url, etc.
  const siteId = resp.data.site_id;
  const adminUrl = resp.data.admin_url; // just for reference
  console.log("createNewNetlifySite: success, siteId =", siteId);
  console.log("createNewNetlifySite: adminUrl =", adminUrl);
...
/**
 *  deployHtmlToNetlify: 
 *  1) Creates a new ephemeral site
 *  2) Zips up your HTML as index.html
 *  3) Deploys to that new site
 *  4) Returns the site’s deploy preview URL 
 */
async function deployHtmlToNetlify(htmlString) {
  // 1) Create the brand-new Netlify site
  const newSiteId = await createNewNetlifySite();
  ...
  // 2) Deploy to the newly-created site
  const deployUrl = `https://api.netlify.com/api/v1/sites/${newSiteId}/deploys`;
  console.log("deployHtmlToNetlify: POST ->", deployUrl);
  ...
  const finalDeployUrl = response.data.deploy_url || response.data.url;
  console.log("deployHtmlToNetlify: SUCCESS, Netlify URL =", finalDeployUrl);

  return finalDeployUrl;
}
...
`;
    ...
    // 6) Deploy the HTML to Netlify (creating new ephemeral site each time)
    ...
    return res.status(200).json({
      success: true,
      netlifyUrl,
      note: "Deployed to a brand-new Netlify site successfully",
    });
  } catch (err) {
