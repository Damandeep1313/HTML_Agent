 ...
 *    npm install express openai axios dotenv adm-zip sharp cloudinary
 *
...
// ----------------------------------------------------
// 0) CONFIGURE CLOUDINARY (for image compression+upload)
// ----------------------------------------------------
cloudinary.config({
...
async function fetchCompressUpload(originalUrl) {
  try {
    ...
    // If something fails, we can either throw or fallback to original URL.
    // We'll throw here:
...
`;
    ...
    if (Array.isArray(parsed.images)) {
      for (let i = 0; i < parsed.images.length; i++) {
        ...
        try {
          ...
          // fallback? or remove it. We'll do fallback to original:
    ...
    return res.status(200).json({
      success: true,
      netlifyUrl,
      note: "Images compressed & uploaded to Cloudinary, then deployed to Netlify.",
    });
  } catch (err) {
